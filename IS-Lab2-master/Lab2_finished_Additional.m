...
surf(reshape(x1,20,20), reshape(x2,20,20), reshape(y_target,20,20), ...
    'FaceColor', 'interp', 'FaceAlpha', 0.6, 'EdgeColor', 'none');
hold on;
...
title('Target vs MLP Output Surface (Overlay)');
xlabel('x1'); 
ylabel('x2'); 
zlabel('y');
...
% Create an interpolating function for the target values.
% This allows us to estimate target outputs at any (x1, x2) point.
F = scatteredInterpolant(x1, x2, y_target);

% Evaluate the target function on the test grid (65x65 points),
% so it matches the resolution of y_pred for comparison.
y_target_interp = F(x1_test, x2_test);

% Create a new figure for the difference surface plot.
figure;

% Plot the difference surface:
%   Z = target - prediction
% reshape(...) converts the long column vectors into 65x65 grids.
% 'EdgeColor','none' removes grid lines for a smooth surface.
surf(reshape(x1_test,65,65), reshape(x2_test,65,65), ...
     reshape(y_target_interp - y_pred,65,65), ...
     'EdgeColor', 'none');

title('Difference Surface (Target − MLP)');
xlabel('x1'); 
ylabel('x2'); 
zlabel('Error');

% Show a color scale indicating magnitude of errors.
colorbar;

% Use the 'jet' colormap for better visual contrast.
colormap jet;
